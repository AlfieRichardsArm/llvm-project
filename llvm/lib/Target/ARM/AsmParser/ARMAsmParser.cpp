...
class ARMAsmParser : public MCTargetAsmParser {
  ...
  bool validatetLDMRegList(const MCInst &Inst, const OperandVector &Operands,
                           unsigned RHSOperandStart, unsigned ListIndex,
                           bool IsARPop = false);
...
#include "ARMGenAsmMatcher.inc"
  ...
  bool processInstruction(MCInst &Inst, const OperandVector &Ops,
                          unsigned RHSOperatorStart, MCStreamer &Out);
...
static unsigned getRHSStart(const OperandVector &Operands) {
  unsigned RHSOperatorStart = 1;
  // Special case for CPS which has a LHS token for possibly storing ie/id
  // variant
  ...
  while (RHSOperatorStart < Operands.size()) {
    auto Op = static_cast<ARMOperand &>(*Operands[RHSOperatorStart]);
...
unsigned findCondCodeInd(const OperandVector &Operands,
                         unsigned RHSOperatorStart) {
  for (unsigned I = 1; I < RHSOperatorStart; ++I) {
...
void ARMAsmParser::cvtThumbMultiply(MCInst &Inst,
  ...
  LLVM_DEBUG(dbgs() << "Start: "; Inst.print(dbgs()); dbgs() << "\n";);
  LLVM_DEBUG(dbgs() << "Ops: [";
             Operands[0]->print(dbgs()); for (unsigned I = 1;
                                              I < Operands.size(); ++I) {
               dbgs() << ", ";
               Operands[I]->print(dbgs());
             } dbgs() << "]\n";);

  unsigned RHSOperatorStart = getRHSStart(Operands);
  unsigned CondI = findCondCodeInd(Operands, RHSOperatorStart);
  unsigned CondOutI = findCCOutInd(Operands, RHSOperatorStart);

  // 2 operand form
  unsigned RegRd = RHSOperatorStart;
  unsigned RegRn = RHSOperatorStart + 1;
  unsigned RegRm = RHSOperatorStart;

  if (Operands.size() == RHSOperatorStart + 3) {
    // If we have a three-operand form, make sure to set Rn to be the operand
    // that isn't the same as Rd.
    if (((ARMOperand &)*Operands[RegRd]).getReg() ==
        ((ARMOperand &)*Operands[RHSOperatorStart + 1]).getReg()) {
      RegRn = RHSOperatorStart + 2;
      RegRm = RHSOperatorStart + 1;
    } else {
      RegRn = RHSOperatorStart + 1;
      RegRm = RHSOperatorStart + 2;
    }
  }
  ...
  LLVM_DEBUG(dbgs() << "End: "; Inst.print(dbgs()); dbgs() << "\n";);
}
...
void ARMAsmParser::cvtThumbBranches(MCInst &Inst,
  ...
  switch(Inst.getOpcode()) {
    ...
    case ARM::tB: {
      ARMOperand &op = static_cast<ARMOperand &>(*Operands[RHSOperatorStart]);
  ...
  ((ARMOperand &)*Operands[RHSOperatorStart]).addImmOperands(Inst, 1);
  if (CondI != 0) {
    ((ARMOperand &)*Operands[CondI]).addCondCodeOperands(Inst, 2);
  } else {
    ARMOperand Op =
...
void ARMAsmParser::cvtMVEVMOVQtoDReg(
  ...
  assert(Operands.size() == RHSOperatorStart + 6);

  ((ARMOperand &)*Operands[RHSOperatorStart]).addRegOperands(Inst, 1); // Rt
  ((ARMOperand &)*Operands[RHSOperatorStart + 1])
      .addRegOperands(Inst, 1); // Rt2
  ((ARMOperand &)*Operands[RHSOperatorStart + 2]).addRegOperands(Inst, 1); // Qd
  ((ARMOperand &)*Operands[RHSOperatorStart + 3])
      .addMVEPairVectorIndexOperands(Inst, 1); // idx
  // skip second copy of Qd in Operands[6]
  ((ARMOperand &)*Operands[RHSOperatorStart + 5])
...
ParseStatus ARMAsmParser::parseFPImm(OperandVector &Operands) {
  ...
  bool isVmovf = false;
  unsigned RHSOperandStart = getRHSStart(Operands);
  for (unsigned I = 1; I < RHSOperandStart; ++I) {
...
bool operandsContainWide(OperandVector &Operands, unsigned RHSOperandStart) {
  for (unsigned I = 0; I < RHSOperandStart; ++I) {
    auto &Op = static_cast<ARMOperand &>(*Operands[I]);
...
void ARMAsmParser::tryConvertingToTwoOperandForm(
    ...
    OperandVector &Operands, unsigned RHSOperatorStart) {

  if (operandsContainWide(Operands, RHSOperatorStart))
    return;
  if (Operands.size() != RHSOperatorStart + 3)
    return;

  const auto &Op3 = static_cast<ARMOperand &>(*Operands[RHSOperatorStart]);
  auto &Op4 = static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 1]);
  ...
  auto &Op5 = static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 2]);
  if (isThumbTwo()) {
    if (Mnemonic != "add")
      return;
  ...
  if (Transform) {
    if (Swap)
      std::swap(Op4, Op5);
    Operands.erase(Operands.begin() + RHSOperatorStart);
  }
}
...
bool ARMAsmParser::shouldOmitVectorPredicateOperand(StringRef Mnemonic,
                                                    OperandVector &Operands,
                                                    unsigned RHSOperandStart) {
  if (!hasMVE() || Operands.size() < RHSOperandStart)
    return true;
...
void ARMAsmParser::fixupGNULDRDAlias(StringRef Mnemonic,
  ...
  if (Mnemonic != "ldrd" && Mnemonic != "strd")
    return;
  if (Operands.size() < RHSOperatorStart + 2)
    return;

  ARMOperand &Op2 = static_cast<ARMOperand &>(*Operands[RHSOperatorStart]);
  ARMOperand &Op3 = static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 1]);
  ...
  Operands.insert(
      Operands.begin() + RHSOperatorStart + 1,
...
bool ARMAsmParser::CDEConvertDualRegOperand(StringRef Mnemonic,
  ...
  if (Operands.size() < 2 + RHSOperatorStart)
    return false;

  StringRef Op2Diag(
      "operand must be an even-numbered register in the range [r0, r10]");

  const MCParsedAsmOperand &Op2 = *Operands[RHSOperatorStart + 1];
  if (!Op2.isReg())
  ...
  const MCParsedAsmOperand &Op3 = *Operands[RHSOperatorStart + 2];
  if (!Op3.isReg() || Op3.getReg() != RNext)
    return Error(Op3.getStartLoc(), "operand must be a consecutive register");

  Operands.erase(Operands.begin() + RHSOperatorStart + 2);
  Operands[RHSOperatorStart + 1] =
      ARMOperand::CreateReg(RPair, Op2.getStartLoc(), Op2.getEndLoc());
  return false;
}

void removeCondCode(OperandVector &Operands, unsigned &RHSOperatorStart) {
  for (unsigned I = 0; I < RHSOperatorStart; ++I)
...
void removeCCOut(OperandVector &Operands, unsigned &RHSOperatorStart) {
  for (unsigned I = 0; I < RHSOperatorStart; ++I)
...
void removeVPTCondCode(OperandVector &Operands, unsigned &RHSOperatorStart) {
  for (unsigned I = 0; I < RHSOperatorStart; ++I)
...
bool ARMAsmParser::ParseInstruction(ParseInstructionInfo &Info, StringRef Name,
  ...
  // This marks the end of the LHS Mnumonic operators and the start of the
  // RHS operators.
  // This is used for indexing into the RHS operators as some of the LHS
  // operators are optional.
  unsigned RHSOperatorStart = Operands.size();
  ...
  if (hasCDE() && MS.isCDEInstr(Mnemonic)) {
    ...
    if (MS.isCDEDualRegInstr(Mnemonic)) {
      bool GotError =
          CDEConvertDualRegOperand(Mnemonic, Operands, RHSOperatorStart);
      if (GotError)
        return GotError;
    }
  }
  ...
  if (hasMVE()) {
    ...
    // operand if that is the case.
      ...
      if (!shouldOmitVectorPredicateOperand(Mnemonic, Operands,
        ...
        if (Mnemonic.starts_with("vcvtt") && RHSOperatorStart > 2) {
          auto Sz1 = static_cast<ARMOperand &>(*Operands[RHSOperatorStart - 2]);
          auto Sz2 = static_cast<ARMOperand &>(*Operands[RHSOperatorStart - 1]);
      ...
      if (shouldOmitVectorPredicateOperand(Mnemonic, Operands,
                                           RHSOperatorStart)) {
        removeVPTCondCode(Operands, RHSOperatorStart);
  ...
  if (!isThumb() && Mnemonic == "blx" &&
      Operands.size() == RHSOperatorStart + 1 &&
      static_cast<ARMOperand &>(*Operands[RHSOperatorStart]).isImm())
    removeCondCode(Operands, RHSOperatorStart);
  ...
  // FIXME: We would really like to be able to tablegen'erate this.
  if (!isThumb() && Operands.size() > RHSOperatorStart + 1 &&
      (Mnemonic == "ldrexd" || Mnemonic == "strexd" || Mnemonic == "ldaexd" ||
       Mnemonic == "stlexd")) {
    bool isLoad = (Mnemonic == "ldrexd" || Mnemonic == "ldaexd");
    unsigned Idx = isLoad ? RHSOperatorStart : RHSOperatorStart + 1;
  ...
  fixupGNULDRDAlias(Mnemonic, Operands, RHSOperatorStart);
  ...
  if (isThumbTwo() && Mnemonic == "sub" &&
      Operands.size() == RHSOperatorStart + 3 &&
      static_cast<ARMOperand &>(*Operands[RHSOperatorStart]).isReg() &&
      static_cast<ARMOperand &>(*Operands[RHSOperatorStart]).getReg() ==
          ARM::PC &&
      static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 1]).isReg() &&
      static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 1]).getReg() ==
          ARM::LR &&
      static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 2]).isImm()) {
...
unsigned getRegListInd(const OperandVector &Operands,
                       unsigned RHSOperatorStart) {
  for (unsigned I = RHSOperatorStart; I < Operands.size(); ++I) {
...
bool ARMAsmParser::validatetLDMRegList(const MCInst &Inst,
  ...
  if (!IsARPop && ListContainsSP)
    return Error(
        Operands[getRegListInd(Operands, RHSOperatorStart)]->getStartLoc(),
...
bool ARMAsmParser::validateLDRDSTRD(MCInst &Inst, const OperandVector &Operands,
  ...
  if (ARMMode) {
    // Rt can't be R14.
    if (Rt == 14)
      return Error(Operands[RHSOperatorStart]->getStartLoc(),
                   "Rt can't be R14");
  ...
  if (Writeback) {
    ...
    if (Rn == Rt || Rn == Rt2) {
      if (Load)
        ...
        return Error(Operands[RHSOperatorStart + 2]->getStartLoc(),
...
bool ARMAsmParser::validateInstruction(MCInst &Inst,
  ...
  case ARM::LDRD:
    if (validateLDRDSTRD(Inst, Operands, /*Load*/ true, /*ARMMode*/ true,
                         /*Writeback*/ false, RHSOperatorStart))
      return true;
    break;
  case ARM::LDRD_PRE:
  case ARM::LDRD_POST:
    if (validateLDRDSTRD(Inst, Operands, /*Load*/ true, /*ARMMode*/ true,
                         /*Writeback*/ true, RHSOperatorStart))
      return true;
    break;
  case ARM::t2LDRDi8:
  ...
  case ARM::t2STRB_POST: {
    ...
    if (Rt == Rn)
      return Error(Operands[RHSOperatorStart + 1]->getStartLoc(),
                   "source register and base register can't be identical");
    return false;
  }
  ...
  case ARM::tLDMIA: {
    ...
    bool HasWritebackToken =
        (static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 1]).isToken() &&
         static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 1])
                 .getToken() == "!");

    unsigned RegStart =
        HasWritebackToken ? RHSOperatorStart + 2 : RHSOperatorStart + 1;
    ...
    if (validatetLDMRegList(Inst, Operands, RHSOperatorStart, 3))
      return true;
    break;
  }
  case ARM::LDMIA_UPD:
  ...
  case ARM::t2STMIA:
  case ARM::t2STMDB:
    if (validatetSTMRegList(Inst, Operands, RHSOperatorStart, 3))
      return true;
    break;
  ...
  case ARM::tMUL:
    ...
    if (Operands.size() == RHSOperatorStart + 3 &&
        (((ARMOperand &)*Operands[RHSOperatorStart]).getReg() !=
         ((ARMOperand &)*Operands[RHSOperatorStart + 2]).getReg()) &&
        (((ARMOperand &)*Operands[RHSOperatorStart]).getReg() !=
         ((ARMOperand &)*Operands[RHSOperatorStart + 1]).getReg())) {
      return Error(Operands[RHSOperatorStart]->getStartLoc(),
  ...
  case ARM::tPOP: {
    ...
    if (validatetLDMRegList(Inst, Operands, RHSOperatorStart, 2, !isMClass()))
      return true;
    break;
  }
  case ARM::tPUSH: {
    ...
    if (validatetSTMRegList(Inst, Operands, RHSOperatorStart, 2))
      return true;
    break;
  }
  case ARM::tSTMIA_UPD: {
    ...
    if (validatetSTMRegList(Inst, Operands, RHSOperatorStart, 4))
      return true;
    break;
  }
  case ARM::tADDrSP:
  ...
  case ARM::tB:
    if (!(static_cast<ARMOperand &>(*Operands[RHSOperatorStart]))
             .isSignedOffset<11, 1>())
      return Error(Operands[RHSOperatorStart]->getStartLoc(),
                   "branch target out of range");
    break;
  case ARM::t2B: {
    int op = (Operands[RHSOperatorStart]->isImm()) ? RHSOperatorStart
                                                   : RHSOperatorStart + 1;
  ...
  case ARM::tCBZ:
  case ARM::tCBNZ: {
    if (!static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 1])
  ...
  case ARM::t2MOVTi16:
    ...
    // to make.
    int i = (Operands[RHSOperatorStart]->isImm()) ? RHSOperatorStart
                                                  : RHSOperatorStart + 1;
  ...
  case ARM::tADDi8: {
    MCParsedAsmOperand &Op = *Operands[RHSOperatorStart + 1];
  ...
  case ARM::tMOVi8: {
    MCParsedAsmOperand &Op = *Operands[RHSOperatorStart];
    if (isARMMCExpr(Op) && !isThumbI8Relocation(Op))
      return Error(Op.getStartLoc(),
  ...
  case ARM::VLDMDIA:
  case ARM::VSTMDIA: {
    ARMOperand &Op = static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 1]);
  ...
  case ARM::MVE_VMULLTu32: {
    if (Operands[RHSOperatorStart]->getReg() ==
        Operands[RHSOperatorStart + 1]->getReg()) {
      return Error(Operands[RHSOperatorStart]->getStartLoc(),
                   "Qd register and Qn register can't be identical");
    }
    if (Operands[RHSOperatorStart]->getReg() ==
        Operands[RHSOperatorStart + 2]->getReg()) {
      return Error(Operands[RHSOperatorStart]->getStartLoc(),
  ...
  case ARM::MVE_VMOV_rr_q: {
    if (Operands[RHSOperatorStart + 2]->getReg() !=
        Operands[RHSOperatorStart + 4]->getReg())
      return Error(Operands[RHSOperatorStart + 2]->getStartLoc(),
                   "Q-registers must be the same");
    if (static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 3])
            .getVectorIndex() !=
        static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 5])
                .getVectorIndex() +
            2)
      return Error(Operands[RHSOperatorStart + 3]->getStartLoc(),
  ...
  case ARM::MVE_VMOV_q_rr: {
    ...
    if (static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 1])
            .getVectorIndex() !=
        static_cast<ARMOperand &>(*Operands[RHSOperatorStart + 3])
                .getVectorIndex() +
            2)
      return Error(Operands[RHSOperatorStart + 1]->getStartLoc(),
                   "Q-register indexes must be 2 and 0 or 3 and 1");
...
unsigned ARMAsmParser::checkTargetMatchPredicate(MCInst &Inst) {
  ...
  if (MCID.TSFlags & ARMII::ThumbArithFlagSetting) {
    ...
    bool IsCPSR = false;
    // Check if the instrduction has CSPR set.
  ...
  case ARM::tMUL:
    ...
    if (Inst.getOperand(0).getReg() != Inst.getOperand(3).getReg())
      return Match_RequiresFirstLastMatch;
    break;
  default:
    break;
  }
...
bool ARMAsmParser::MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  case Match_Success:
    ...
    // so check them here.
    if (validateInstruction(Inst, Operands, RHSOperatorStart)) {
...

    {
      ...
      while (processInstruction(Inst, Operands, RHSOperatorStart, Out))
  ...
  for (NearMissInfo &I : reverse(NearMissesIn)) {
    ...
    case NearMissInfo::NearMissPredicate: {
      ...
      case Match_RequiresFlagSetting:
        ...
        break;
      case Match_RequiresFirstLastMatch:
